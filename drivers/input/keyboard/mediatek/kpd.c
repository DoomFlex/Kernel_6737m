...
static struct device_node *homekey_irq_node;
int homekey_irq;
#endif
...
void kpd_homekey_handler(unsigned long data)
{
  ...
  /* for detecting the return to old_state */  
  irq_set_irq_type(homekey_irq, old_state?IRQF_TRIGGER_HIGH:IRQF_TRIGGER_LOW);
  enable_irq(homekey_irq);
}

static irqreturn_t kpd_homekey_eint_handler(int irq, void *desc)
{
    disable_irq_nosync(homekey_irq);
    tasklet_schedule(&kpd_homekey_tasklet);
    return IRQ_HANDLED;
}

int mt_eint_homekey_register(void)
{
/*
  mt_set_gpio_mode(GPIO_HOME_EINT_PIN, GPIO_HOME_EINT_PIN_M_EINT);
  mt_set_gpio_dir(GPIO_HOME_EINT_PIN, GPIO_DIR_IN);
  //mt_set_gpio_pull_enable(GPIO_KPD_MUTEKEY_EINT_PIN, GPIO_PULL_DISABLE);
  mt_set_gpio_pull_enable(GPIO_HOME_EINT_PIN, GPIO_PULL_ENABLE);
  mt_set_gpio_pull_select(GPIO_HOME_EINT_PIN, GPIO_PULL_UP);

  mt_eint_set_sens(CUST_EINT_HOME_NUM, CUST_EINT_HOME_TYPE);
  mt_eint_set_hw_debounce(CUST_EINT_HOME_NUM, CUST_EINT_HOME_DEBOUNCE_EN);

  mt_eint_registration(CUST_EINT_HOME_NUM, true,kpd_homekey_eint_handler, false);
*/
    u32 ints[2] = {0, 0};
    
    homekey_irq_node = of_find_compatible_node(NULL, NULL, "mediatek, HOME-eint");
    
    if(homekey_irq_node)
    {
		of_property_read_u32_array(homekey_irq_node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_request(ints[0], "homekey");
		gpio_set_debounce(ints[0], ints[1]);
        /*
		pinctrl_select_state(hallctrl, irqpins_cfg);
		pr_err("ints[0] = %d, ints[1] = %d!!\n", ints[0], ints[1]);
        */
		homekey_irq = irq_of_parse_and_map(homekey_irq_node, 0);
		pr_err("homekey_irq = %d\n", homekey_irq);
		if (!homekey_irq) {
			pr_err("irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
      
		if (request_irq(homekey_irq, kpd_homekey_eint_handler,IRQF_TRIGGER_LOW , "HOME-eint", NULL)) {
