...
//#define DTS_PROBE
#define ENABLE_SPI_FIFO_TRANSFER

#ifdef HAS_RESET_PIN
#define ENABLE_SPI_FREQUENCY_CALIBRTION
#endif

#define VERSION                         "cdfinger version 2.2"
#define DEVICE_NAME                     "fpsdev0"
#define SPI_DRV_NAME                    "cdfinger"
#define CDFINGER_HEIGHT                  96
#define CDFINGER_WIDTH                   112
...
#else
#define CDFINGER_RD_PIX_SIZE             (16*1024)
#endif
#define CDFINGER_SPI_CALIBRATION_SIZE    (CDFINGER_RD_PIX_SIZE - 1500)
...
#define CDFINGER_CHECK_INETRRUPT         _IO(CDFINGER_IOCTL_MAGIC_NO, 6)
...
#define KEY_INTERRUPT                   KEY_F10

enum work_mode {
	CDFINGER_MODE_NONE,
	CDFINGER_INTERRUPT_MODE,
	CDFINGER_KEY_MODE,
	CDFINGER_FINGER_UP_MODE
};
...
#ifdef ENABLE_SPI_FIFO_TRANSFER
...
//	mutex_lock(&cdfinger->transfer_lock);
	if(tx[0] == 0x14 && tx[0] == cdfinger->last_transfer)
	{
...
#ifdef HAS_RESET_PIN
static void cdfinger_reset(void)
{
...
static int cdfinger_config_interrupt(struct cdfinger_data *cdfinger, u8 arg)
{
	...
	static u8 reg21_tx[7] = {0x21,0x66,0x66,0xf5,0x00,0x00,0x70};
...
static int cdfinger_check_interrupt(struct cdfinger_data *cdfinger)
{
	...
	for(regval=0xf0; regval>=0xa0; regval-=16)
	{
        ...
        CDFINGER_FUNCTION("11111  %x   %d\n",regval,cdfinger->cdfinger_interrupt);
...
static int cdfinger_mode_init(struct cdfinger_data *cdfinger, uint8_t arg, enum work_mode mode)
{
	...
	{
		CDFINGER_ERR("interrupt config failed! mode=%d\n", mode);
		return -1;
	}
...
static int cdfinger_read_image(struct cdfinger_data *cdfinger, uint32_t timeout)
{
	...
	ret = spi_send_cmd(cdfinger, cdfinger->imagetxcmd, cdfinger->imagerxpix, CDFINGER_RD_PIX_SIZE);
	if (ret != 0){
...
int cdfinger_report_key(struct cdfinger_data *cdfinger)
{
	CDFINGER_FUNCTION("enter\n");
...
static long cdfinger_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case CDFINGER_REPORT_KEY:
			ret = cdfinger_report_key(cdfinger);
		default:
			ret = -ENOTTY;
			break;
	}
	mutex_unlock(&cdfinger->buf_lock);
	CDFINGER_DBG("return value: %d\n", ret);
	CDFINGER_FUNCTION("exit\n");
