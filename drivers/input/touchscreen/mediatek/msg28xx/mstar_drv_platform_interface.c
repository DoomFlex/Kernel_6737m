...
/* Vanzo:wuchuanrong on: Fri, 09 Dec 2016 17:39:44 +0800
 * TODO: add note for firmware version
 */
u16 V1,V2;
u8 version[20] = {0};
static ssize_t msg2xxx_fwversion_show(struct device *dev,struct device_attribute *attr,char *buf)
{
  ...
  DrvFwCtrlGetCustomerFirmwareVersion(&V1, &V2, &version[0]);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
    ...
    /* Vanzo:wuchuanrong on: Fri, 09 Dec 2016 17:35:59 +0800
     * TODO: create node
     */

    if (sysfs_create_group(&pClient->dev.kobj, &msg2xxx_attribute_group)) {
      TPD_DMESG("create attr fail\n");
      return -1;
    }
    // End of Vanzo: wuchuanrong
