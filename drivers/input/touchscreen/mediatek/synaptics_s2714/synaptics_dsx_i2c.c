...
static DEVICE_ATTR(gesture, 0664, synaptics_rmi4_wake_gesture_show, synaptics_rmi4_wake_gesture_store);
...
static int synaptics_rmi4_i2c_read(struct synaptics_rmi4_data *rmi4_data,unsigned short addr, unsigned char *data, unsigned short length)
{
	u8 retval = 0;
	u8 retry = 0;
	u8 *pData = data;
	...
	while (left_len > 0) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			retval = i2c_master_send(rmi4_data->i2c_client, pData, 1);
			...
			if (left_len > 8)
				retval = i2c_master_recv(rmi4_data->i2c_client, pData, 8);
			else
				retval = i2c_master_recv(rmi4_data->i2c_client, pData, left_len);
 ...
 * a retry mechanism.
 */
static int synaptics_rmi4_i2c_write(struct synaptics_rmi4_data *rmi4_data,unsigned short addr, unsigned char *data, unsigned short length)
{
	u8 retval = 0;
	u8 retry = 0;
	u8 *pData = data;
	...
	retval = synaptics_rmi4_set_page(rmi4_data, addr);
	if (retval != PAGE_SELECT_LEN) {
		printk("synaptics_rmi4_set_page fail, retval = %d\n", retval);
		retval = -EIO;
		goto exit;
	}
	...
	while (left_len > 0) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			if (left_len > 4) {
				...
				retval = i2c_master_send(rmi4_data->i2c_client, buf, 5);
			} else {
				memcpy(buf + 1, pData, left_len);
				retval = i2c_master_send(rmi4_data->i2c_client, buf, left_len + 1);
			}
...
static int synaptics_rmi4_irq_enable(void)
{
	...
	if (node) {
		...
		printk("xxxxxxxxxxxxxxxxxx touch_irq number %d\n", touch_irq);
                //ret =request_irq(touch_irq, (irq_handler_t) tpd_eint_interrupt_handler, IRQF_TRIGGER_PROBE, GTP_DRIVER_NAME, NULL);
		...
		const struct i2c_device_id *dev_id)
{
	...
	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);  //"synaptics-tpd");  // TPD_DEVICE
	if ( IS_ERR(thread) ) {
...
static struct tpd_driver_t tpd_device_driver = {
	.tpd_device_name = GTP_DRIVER_NAME,  //"synaptics",
