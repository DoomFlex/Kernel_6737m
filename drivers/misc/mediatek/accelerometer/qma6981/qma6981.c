...
static struct acc_hw hw_s;
static struct acc_hw* hw = &hw_s;
struct QMA6981_data{
...
/* for custom direction by adb shell */
...
*/
	...
	{
		qma6981->hw->direction = direction;
		hwmsen_get_convert(qma6981->hw->direction, &qma6981->cvt);
	}
	else
	{
		printk(KERN_ERR "invalid content: '%s', length = %ld\n", buf, count);
...
static int qma6981_virtual_motion_input_init(void)
{
	struct input_dev *dev;
	int ret;
	dev = input_allocate_device();
...
static const struct of_device_id accel_of_match[] = {
    {.compatible = "mediatek,GSENSOR_QMA6981"},
    {},
};
