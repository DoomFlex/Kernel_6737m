...
struct pinctrl *aw3644ctrl = NULL;
struct pinctrl_state *aw3644_hwen_high = NULL;
struct pinctrl_state *aw3644_hwen_low = NULL;
...
{
	int ret = 0;
	aw3644ctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(aw3644ctrl)) {
		dev_err(&pdev->dev, "Cannot find aw3644 pinctrl!");
		ret = PTR_ERR(aw3644ctrl);
		printk("%s devm_pinctrl_get fail!\n", __func__);
	}
	
	aw3644_hwen_high = pinctrl_lookup_state(aw3644ctrl, "hwen_high");
	if (IS_ERR(aw3644_hwen_high)) {
		ret = PTR_ERR(aw3644_hwen_high);
		printk("%s : pinctrl err, aw3644_hwen_high\n", __func__);
	}

	aw3644_hwen_low = pinctrl_lookup_state(aw3644ctrl, "hwen_low");
	if (IS_ERR(aw3644_hwen_low)) {
		ret = PTR_ERR(aw3644_hwen_low);
		printk("%s : pinctrl err, aw3644_hwen_low\n", __func__);
	}

    subflashlight_en_high = pinctrl_lookup_state(aw3644ctrl, "subflashlight_high");
    if (IS_ERR(subflashlight_en_high)) {
        ret = PTR_ERR(subflashlight_en_high);
        printk("%s : init err, subflashlight_en_high\n", __func__);
    }

    subflashlight_en_low = pinctrl_lookup_state(aw3644ctrl, "subflashlight_low");
    if (IS_ERR(subflashlight_en_low)) {
        ret = PTR_ERR(subflashlight_en_low);
        printk("%s : init err, subflashlight_en_low\n", __func__);
    }
...
{
	printk("%s enter\n", __func__);
	pinctrl_select_state(aw3644ctrl, aw3644_hwen_low);
	msleep(5);
	pinctrl_select_state(aw3644ctrl, aw3644_hwen_high);
	msleep(10);
	printk("%s exit\n", __func__);
...
unsigned char subflashlight_on(void)
{
	printk("%s enter\n", __func__);
	pinctrl_select_state(aw3644ctrl, subflashlight_en_high);
	printk("%s exit\n", __func__);
	return 0;
}

unsigned char subflashlight_off(void)
{
	printk("%s enter\n", __func__);
	pinctrl_select_state(aw3644ctrl, subflashlight_en_low);
	printk("%s exit\n", __func__);
	return 0;
}
...
int FL_Enable(void)
{
	...
	flashEnable_aw3644_1();
	return flashEnable_aw3644_2();
}
...
int FL_Disable(void)
{
	PK_DBG(" FL_Disable line=%d\n", __LINE__);
	 flashDisable_aw3644_1();
	return flashDisable_aw3644_2();
}
