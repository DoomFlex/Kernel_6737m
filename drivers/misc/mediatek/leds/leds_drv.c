...
/* Vanzo:yangyang on: Mon, 20 Jun 2016 19:45:55 +0800
 * TODO: add button-backlight by GPIO
  */
        printk("yang cust_led_list.mode is %s,cust_led_list.name is %s\n",cust_led_list[i].mode,cust_led_list[i].name);
		if ((cust_led_list[i].mode == MT65XX_LED_MODE_GPIO) && (strcmp(cust_led_list[i].name,"button-backlight") == 0)) {
          printk("yang leds_drv enter gpio mode!!\n");
          button_gpio_init(pdev);
        }
// End of Vanzo:        * *yangyang
...
#ifdef CONFIG_OF
static const struct of_device_id button_backlight_match_table[] = {
...
static struct platform_driver mt65xx_leds_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
    .of_match_table = button_backlight_match_table,
#endif
		   },
