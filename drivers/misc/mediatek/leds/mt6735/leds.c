...
struct pinctrl_state *button_h = NULL;
struct pinctrl_state *button_l = NULL;

int button_gpio_init(struct platform_device *pdev)
{
    int ret = 0;
    printk("%s line %d \n",__func__,__LINE__);
    ...
    if (IS_ERR(buttonctrl)) {
      dev_err(&pdev->dev, "Cannot find button pinctrl!");
      printk(KERN_ERR"--------Cannot find button pinctrl -----------\n");
      ret = PTR_ERR(buttonctrl);
    }

    button_h = pinctrl_lookup_state(buttonctrl, "buttonoutput1");
    if (IS_ERR(button_h)) {
      ret = PTR_ERR(button_h);
      printk(KERN_ERR"%s : pinctrl err,buttonoutput1\n", __func__);
    }

    button_l = pinctrl_lookup_state(buttonctrl, "buttonoutput0");
    if (IS_ERR(button_l)) {
      ret = PTR_ERR(button_l);
      printk(KERN_ERR"%s : pinctrl err,buttonoutput0\n", __func__);
    }
	...
	case MT65XX_LED_MODE_GPIO:
        ...
        if ((strcmp(cust->name, "button-backlight") == 0)) {
          ...
          */
          if(level == 0)
          {
          pinctrl_select_state(buttonctrl,button_l);
          }
          else
          {
          pinctrl_select_state(buttonctrl,button_h);
          }
          return 1;
        }
        else
