...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xf7, 0x01); //pll enable
	write_cmos_sensor(0xf8, 0x11); //0f//0e//0d //pll mode
	write_cmos_sensor(0xf9, 0xaa);
	...
	write_cmos_sensor(0xe7, 0xc0); //sram读写使能
	...
	write_cmos_sensor(0x0a, 0x14);   // 12   modify 1123                                                                                                  
	write_cmos_sensor(0x0b, 0x00); 
	write_cmos_sensor(0x0c, 0x10); //14//14//00 //04
	write_cmos_sensor(0x0d, 0x07); 
	write_cmos_sensor(0x0e, 0xa0); 
	write_cmos_sensor(0x0f, 0x0a); 
	write_cmos_sensor(0x10, 0x30);  //54 
	write_cmos_sensor(0x17, MIRROR);//Don't Change Here!!!                                          
	write_cmos_sensor(0x18, 0x02); 
	write_cmos_sensor(0x19, 0x0a);
	write_cmos_sensor(0x1a, 0x1b); //13
	write_cmos_sensor(0x1c, 0x0c); //10 20151013//01//19
	write_cmos_sensor(0x1d, 0x19); //0f//14//18//1f
	write_cmos_sensor(0x21, 0x16); //18 20150929
	write_cmos_sensor(0x24, 0xb0); 
	write_cmos_sensor(0x25, 0xc1); //46
	write_cmos_sensor(0x27, 0x64); //44 20150929 [5:4]txl_drv_mode
	write_cmos_sensor(0x29, 0x28); //24 20151013 
	write_cmos_sensor(0x2a, 0xc3); //c4 20151013//c1 20150929
	write_cmos_sensor(0x31, 0x40);
	write_cmos_sensor(0x32, 0xf8); //fe 20151015//f8 20151013 [2:0]txlow_r//da//ba
	write_cmos_sensor(0xcd, 0xca); 
	write_cmos_sensor(0xce, 0xff); 
	write_cmos_sensor(0xcf, 0x70); //10 20150929 [6:4]ctx
	write_cmos_sensor(0xd0, 0xd2); //b2 20150929 [6:4]da_vref
	write_cmos_sensor(0xd1, 0xa0); //b4 20150929 [6:4]rsgh_r
	write_cmos_sensor(0xd3, 0x23); 
	write_cmos_sensor(0xd8, 0x12); 
	write_cmos_sensor(0xdc, 0xb3); //16//14
	write_cmos_sensor(0xe1, 0x1b); //15 20151013//14 20150929 [3:0]post_tx_width
	write_cmos_sensor(0xe2, 0x00); //20  modify 1123  
	write_cmos_sensor(0xe4, 0x78);                                                                                                         
	write_cmos_sensor(0xe6, 0x1f); //10 20151013//04 20150929 [4:0]ramp_offset 
	write_cmos_sensor(0xe7, 0xc0); 
	write_cmos_sensor(0xe8, 0x01); //16//14
	write_cmos_sensor(0xe9, 0x02); //18
	write_cmos_sensor(0xec, 0x01); //20150929
	write_cmos_sensor(0xed, 0x02); //20150929
	/*ISP*/
	write_cmos_sensor(0x80, 0x50); //20150929
	...
	/*Gain*/
	write_cmos_sensor(0x99, 0x00);   //00 2015/10/12
	write_cmos_sensor(0x9a, 0x08);   //01 2015/10/12                                     
	write_cmos_sensor(0x9b, 0x10);   //02 2015/10/12                                 
	write_cmos_sensor(0x9c, 0x18);   //03 2015/10/12                              
	write_cmos_sensor(0x9d, 0x19);   
	write_cmos_sensor(0x9e, 0x1a);   
	write_cmos_sensor(0x9f, 0x1b);   
	write_cmos_sensor(0xa0, 0x1c);   //1c 2015/10/12
	write_cmos_sensor(0xb0, 0x50);
	...
	write_cmos_sensor(0xab, 0x00); //20150929//dark offset //2015/10/12  ini :02
	write_cmos_sensor(0xac, 0x30); //20150925
	...
	/*Degrid*/
	//write_cmos_sensor(0x8a, 0x0a); //0b //[3:2]去格子
