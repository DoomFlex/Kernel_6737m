...
static struct epl_optical_sensor epl_sensor;
static struct i2c_client *epl_sensor_i2c_client = NULL;
/*
static struct epl_raw_data
{
    u8 raw_bytes[PACKAGE_SIZE];
};
static epl_raw_data	gRawData;
*/
static u8 raw_bytes[PACKAGE_SIZE];
...
static int epl_sensor_I2C_Read(struct i2c_client *client, uint8_t regaddr, uint8_t bytecount)
{
    ...
    while(bytecount>0)
    {
        ...
        for(retry = 0; retry < I2C_RETRY_COUNT; retry++)
        {
            ...
            ret = i2c_master_recv(client, &raw_bytes[read_count], rx_count);
...
static void write_global_variable(struct i2c_client *client)
{
    ...
    epl_sensor.revno = raw_bytes[0] | raw_bytes[1] << 8;
...
int epl259x_init_device(struct i2c_client *client)
{
...
int epl_sensor_read_hs(struct i2c_client *client)
{
    ...
    epl_sensor.hs.raw = (raw_bytes[1]<<8) | raw_bytes[0];
...
static int epl_sensor_init_client(struct i2c_client *client)
{
    ...
    if((err = epl259x_init_device(client)) != 0)
    {
        APS_ERR("init dev: %d\n", err);
        return err;
    }
...
static ssize_t epl_sensor_store_integration(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            APS_LOG("%s: 0x03 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.ps.integration_time | epl_sensor.ps.gain,raw_bytes[0]);
            break;
        ...
        case EPL_MODE_ALS: //als
...
#endif
            {
                ...
                APS_LOG("%s: 0x01 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.als.integration_time | epl_sensor.als.gain, raw_bytes[0]);
            }
            break;
...
static ssize_t epl_sensor_store_adc(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    switch (epl_sensor.mode)
    {
        case EPL_MODE_PS:
            ...
            APS_LOG("%s: 0x04 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.ps.adc | epl_sensor.ps.cycle, raw_bytes[0]);
            break;
        ...
        case EPL_MODE_ALS: //als
            ...
            APS_LOG("%s: 0x02 = 0x%x (0x%x)\n", __FUNCTION__, epl_sensor.als.adc | epl_sensor.als.cycle, raw_bytes[0]);
            break;
    }
    epl_sensor_update_mode(obj->client);
...
/*CTS --> S_IWUSR | S_IRUGO*/
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(elan_status,					S_IRUGO, epl_sensor_show_status,  	  		NULL										);
static DRIVER_ATTR(elan_reg,    				S_IRUGO, epl_sensor_show_reg,   				NULL										);
...
/*----------------------------------for factory mode test---------------------------------------*/
		...
		case ALSPS_SET_PS_THRESHOLD:
            ...
            obj->hw->ps_threshold_low = threshold[1]-100;
